...
namespace Slate{
	...
	public class ActionClipInspector : Editor {
		...
		void ShowErrors(){
			...
			if (!action.isValid){
	            EditorGUILayout.HelpBox("The clip is currently invalid. Please make sure the required settings are set.", MessageType.Error);
	            GUILayout.Space(5);
	            return;
			}
		}
		...
		void ShowInOut(){
			...
			var previousClip = action.parent.children.Where(a => a != action && a.startTime < action.startTime).LastOrDefault();
			...
			var nextClip = action.parent.children.Where(a => a != action && a.endTime > action.endTime).FirstOrDefault();
